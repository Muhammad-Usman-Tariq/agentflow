# DigitalSofts AI Platform

[![DigitalSofts AI Platform](https://img.shields.io/badge/DigitalSofts-AI_Development_Platform-e85d3a?style=for-the-badge)](https://digitalsofts.ai)

Welcome to **DigitalSofts AI Platform**, an advanced full-stack AI development workspace equipped with Google Stitch Warm Minimalism UI, multi-agent AI engine, PostgreSQL database persistence, JWT authentication, and automated social media deployment publishing.

---

## 🚀 Tech Stack Overview

- **Frontend**: Remix (React) + Cloudflare Workers + UnoCSS + SCSS + Stitch Warm Minimalism Design System
- **Backend**: Node.js + PostgreSQL + JWT Cookie Sessions
- **AI Multi-Agent System**: 7 Specialized Autonomous Agents (Planner, Architect, Coder, Reviewer, Tester, Deployer, Social Publisher)
- **Deployment Hub**: Vercel, Netlify, GitHub, GitLab
- **Social Media Integrations**: X (Twitter), LinkedIn, Facebook, Instagram

---

## 🛠️ Key Features & Modules

### 1. 🎨 Warm Minimalism Stitch UI System
- Responsive workspace with coral `#e85d3a` branding, custom typography (JetBrains Mono), glassmorphism cards, and dark/light theme support.
- Fully functional drawer sidebar with recent chat history search, new chat actions, and connector configuration modal.

### 2. 🔐 Complete Authentication & Guest Mode
- **JWT & Cookie Session Security** (`app/lib/auth/`)
- User Signup (`/auth/signup`) and Login (`/auth/login`) backed by PostgreSQL.
- **Guest Mode** (`/guest`) for instant AI prototyping without account requirements.
- Full session destruction and welcome landing screen redirect on signout.

### 3. 🤖 Multi-Agent AI System (`app/lib/agents/`)
- 7 parallel AI agents that plan code architecture, generate full-stack files, review diffs, run terminal commands, and handle automated deployment pipelines.
- Real-time execution status tracking overlay (`AgentProgress.tsx`).

### 4. 🌐 Deploy & Social Integrations Hub (`DeployHub.tsx`)
- **Deployment Providers**: One-click deployment settings for Vercel, Netlify, GitHub, and GitLab.
- **Social Media Connections**: Automated release announcements across **X (Twitter)**, **LinkedIn**, **Facebook**, and **Instagram**.
- **Credentials Manager**: Interactive credentials form for storing API keys, OAuth tokens, and page IDs directly into PostgreSQL.

### 5. 💻 Interactive Code Workbench
- Multi-file editor, live preview, diff viewer, and integrated web container terminal.
- Quick action tabs in header (**Overview**, **Agent / Code**, **Terminal**, **Preview**) for drawer control.
- One-click close button `(X)` to hide the workbench and return to full-screen chat.
...
# 3. Start PostgreSQL Database & Migrations
...
- [Setup & Installation](#local-setup--development-guide)
- [Configuring API Keys](#configuring-api-keys-and-providers)
...
##set tunnel typo fix application second try now third try
